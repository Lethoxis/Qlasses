...
tag @a[tag=siphon,scores={lc=1..}] add spawnProjectile
function code:misc/player_spawn_projectile
execute as @a[tag=siphon,scores={lc=1..}] at @s run tag @e[tag=projectile,scores={life=0},sort=nearest,limit=1] add siphon

execute as @a[tag=siphon,scores={lc=1..}] at @s run playsound entity.generic.eat ambient @a ~ ~ ~ 1 0
execute as @a[tag=siphon,scores={lc=1..}] at @s run playsound ui.loom.select_pattern ambient @a ~ ~ ~ 1 0
