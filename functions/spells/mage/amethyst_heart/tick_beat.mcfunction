...
execute as @e[tag=amethyst_heart_beat,scores={life=1}] at @s run summon area_effect_cloud ~ ~ ~ {Particle:{type:"end_rod"},Radius:0f,RadiusPerTick:.7f,Duration:11}
execute as @e[tag=amethyst_heart_beat,scores={life=1}] at @s run summon area_effect_cloud ~ ~ ~ {Particle:{type:"dust",color:[0.663, 0.000, 0.831],scale:1},Radius:0f,RadiusPerTick:.7f,Duration:11}
