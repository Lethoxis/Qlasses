...
execute as @a[tag=amethyst_storm,scores={lc=1..}] at @s run ride @e[tag=amethyst_storm_vfx,tag=new,sort=nearest,limit=1] mount @e[tag=projectile,tag=amethyst_storm,scores={life=0},sort=nearest,limit=1]
tag @e[tag=amethyst_storm_vfx,tag=new] remove new
execute as @a[tag=amethyst_storm,scores={lc=1..}] at @s run execute as @e[tag=projectile,scores={life=0},sort=nearest,limit=1] run data merge entity @s {transformation:{left_rotation:[-.5f,0f,0f,1f],right_rotation:[0f,0f,.5f,1f],translation:[0f,0f,0f],scale:[.6,.6,.6]},item:{id:"minecraft:amethyst_block",Count:1b}}


######## VSFX
execute as @a[tag=amethyst_storm,scores={lc=1..}] at @s run particle dust .6 .3 .7 1 ~ ~7 ~ .1 3 .1 .3 200
execute as @a[tag=amethyst_storm,scores={lc=1..}] at @s run particle dust 1 .9 .2 1 ~ ~7 ~ .1 3 .1 .3 200
execute as @a[tag=amethyst_storm,scores={lc=1..}] at @s run particle cloud ~ ~7 ~ .02 3 .02 .02 80
